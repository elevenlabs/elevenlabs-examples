...
"training": Tell the user to create the agent's knowledge base by uploading documents, inserting text, or submitting URLs to public websites with information that should be available to the agent. Wait patiently without talking to the user. Only when the user confirms that they've provided everything then proceed to the next step.
