...
- Use a simple default name and default system prompt suitable for a general demo assistant.
...
- If you want tentative agent text, use `onDebug` for debug-style events rather than forcing all transcript rendering through raw socket-event parsing.
- It is fine to style tentative or partial text differently, but do not discard prior turns when a new message arrives.
- The transcript should work for agents created by this app. If a user pastes an older external agent id, validate that it exists, but note in code comments or logic that transcript availability still depends on that agent having the required client events enabled.
